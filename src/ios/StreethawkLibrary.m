...
- (void)streethawkinit:(CDVInvokedUrlCommand *)command
{
    ...
    //read app key from Info.plist, which is required when add plugin.
    NSString *appKey = [[NSBundle mainBundle] objectForInfoDictionaryKey:@"APP_KEY"];
    if (appKey != nil && appKey.length > 0)
    {
        [StreetHawk registerInstallForApp:appKey withDefaultHostRootUrl:nil withApnsMode:StreetHawk.apnsMode/*setup by apnsModeExtForPhonegap*/ withDebugMode:StreetHawk.isDebugMode enableCrashReport:YES/*always enable for phonegap*/ withiTunesId:StreetHawk.itunesAppId];
    ...
    {
        pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_ERROR messageAsString:@"No APP_KEY found. Add plugin again with --variable APP_KEY=<YOUR_APPLICATIONS_APP_KEY>."];
    }
...
- (void)currentPage:(CDVInvokedUrlCommand *)command
{
    ...
    if (command.arguments.count == 1)
    {
        if ([command.arguments[0] isKindOfClass:[NSString class]] && ((NSString *)command.arguments[0]).length > 0)
        {
            ...
            [StreetHawk sendLog:LOG_DOMAIN_CUSTOM withCode:LOG_CODE_VIEW_ENTER withComment:page];
            pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
        }
        else
        {
...
- (void)shSendSimpleFeedback:(CDVInvokedUrlCommand *)command
{
    ...
    if (command.arguments.count == 2)
    {
        if ([command.arguments[0] isKindOfClass:[NSString class]] && [command.arguments[1] isKindOfClass:[NSString class]])
        {
            ...
            [StreetHawk shFeedback:nil needInputDialog:NO withTitle:title withMessage:message];
            pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
        }
        else
        {
        ...
        for (int i = 0; i < arrayDicts.count; i ++)
        {
            if ([arrayDicts[i] isKindOfClass:[NSDictionary class]])
            {
                ...
                if (dict.allKeys.count == 1)
                {
                    ...
                    {
                        StreetHawkFriendlyNameObject *obj = [[StreetHawkFriendlyNameObject alloc] init];
...
{
    //empty, nothing to do in iOS as SDK has handle it.
}
...
- (void)shSetApnsMode:(CDVInvokedUrlCommand *)command
{
    ...
    if (command.arguments.count == 1)
    {
        ...
        {
            StreetHawkApnsMode apnsMode = StreetHawkApnsMode_None;
            int apnsModeValue = [command.arguments[0] intValue];
            if (apnsModeValue == 1)
            {
                apnsMode = StreetHawkApnsMode_Dev;
            }
            else if (apnsModeValue == 2)
            {
                apnsMode = StreetHawkApnsMode_Prod;
            }
#pragma GCC diagnostic push
#pragma clang diagnostic push
#pragma GCC diagnostic ignored "-Wundeclared-selector"
#pragma clang diagnostic ignored "-Wundeclared-selector"
            //call private hidden function, only extended for Phonegap, not for normal usage.
            [StreetHawk performSelector:@selector(setApnsModeExtForPhonegapValue:) withObject:[NSNumber numberWithInt:apnsMode]];
...
{
    int ret;
    switch (StreetHawk.apnsMode)
    {
        case StreetHawkApnsMode_Dev:
            ret = 1;
            break;
        case StreetHawkApnsMode_Prod:
            ret = 2;
            break;
        default:
            ret = 0;
...
- (void)shSetDefaultPushNotificationSupport:(CDVInvokedUrlCommand *)command
{
    ...
    if (command.arguments.count == 1)
    {
        if ([command.arguments[0] respondsToSelector:@selector(boolValue)])
        {
            StreetHawk.isDefaultPushNotificationEnabled = [command.arguments[0] boolValue];
            pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
        }
        else
        {
...
- (void)shSetIsPushNotificationEnabled:(CDVInvokedUrlCommand *)command
{
    ...
    if (command.arguments.count == 1)
    {
        if ([command.arguments[0] respondsToSelector:@selector(boolValue)])
        {
            StreetHawk.isPushNotificationEnabled = [command.arguments[0] boolValue];
            pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK];
        }
        else
        {
...
{
    CDVPluginResult *pluginResult = [CDVPluginResult resultWithStatus:CDVCommandStatus_OK messageAsBool:StreetHawk.isPushNotificationEnabled];
    [self.commandDelegate sendPluginResult:pluginResult callbackId:command.callbackId];
}
